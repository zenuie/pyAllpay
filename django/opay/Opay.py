...
from .models import OpaySetting
...
class Opay():
    ...
    test_url = setting.AIO_SANDBOX_SERVICE_URL
    if is_sandbox == False:
        service_url = setting.AIO_SANDBOX_SERVICE_URL
    else:
        service_url = setting.AIO_SERVICE_URL
    ...
    def __init__(self, payment_conf, service_method='post'):
        ...
        self.HASH_KEY = setting.HASH_KEY
        self.HASH_IV = setting.HASH_IV
    ...
    def check_out(self):
        sorted_dict = list(sorted(self.Opay_parameter.items()))
    ...
    def checkout_feedback(cls, post):
        ...
        try:
            ...
            sz_confirm_mac_value = "HashKey=" + setting.HASH_KEY

            for val in sorted_returns:
                sz_confirm_mac_value = "".join((str(sz_confirm_mac_value), "&", str(val[0]), "=", str(val[1])))

            sz_confirm_mac_value = "".join((sz_confirm_mac_value, "&HashIV=", setting.HASH_IV))
